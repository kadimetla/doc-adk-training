...
- **WebSockets vs. SSE for Voice:** While SSE (used in the previous lab) is excellent for one-way streaming (server to client), WebSockets are necessary for voice applications because they provide a **full-duplex** (bidirectional) channel. This allows the client to continuously stream microphone audio *to* the server while simultaneously receiving the agent's audio and text responses *from* the server, which is not possible with SSE.
- **Handling Audio Playback:** To play back audio from the agent, the `websocket.onmessage` handler would need to be modified. It would check for messages with a `mime_type` of `audio/mp3`, decode the Base64-encoded audio data into a binary format, and then use the browser's Web Audio API (`AudioContext`) to decode and play the audio chunk.